...
- **IP 地理归属查询**：~~接入 ip-api.com 免费接口，返回国家、省份、城市、运营商、经纬度信息，支持中文~~ 已使用 GeoLite2 。
...
- **目的**：~~ip-api.com 免费版限速 45 次/分钟，同一 IP（尤其是 8.8.8.8 等公共 DNS）会被多用户触发，缓存避免重复请求~~，已使用 GeoLite2 。
